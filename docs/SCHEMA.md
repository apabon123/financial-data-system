...
This document outlines the structure of the tables within the `data/financial_data.duckdb` database.

## Core Data Tables

These tables are central to the market data storage and continuous contract generation.
...
## Other Tables (Less Core to Current Update/Inspection Flow)
...
*   Account Tables (`account_balances`, `positions`, `orders`, `trades`, etc.): (Future Use) Intended for storing brokerage account information, likely populated from TradeStation API. Not currently part of the core data update flow.

## Views

*   `daily_bars`: Convenience view filtering `market_data` for `interval_unit = 'daily'`.
*   *(Other views like `weekly_bars`, `monthly_bars` may exist but might need updates if aggregation logic is complex or specific fields are missing).*

## Indexes

Appropriate indexes are assumed to be created on primary keys and frequently queried columns (e.g., `timestamp`, `symbol`) for performance. Refer to database initialization scripts if specific index definitions are needed.
